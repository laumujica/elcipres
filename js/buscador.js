...
(function () {
  var INDEX_URL = new URL('../search-index.json', document.currentScript.src).href;
  var indice = null;
  var cargando = null;
  ...
  function initBuscador() {
    ...
    if (wrap && !wrap.classList.contains('buscador-wrap--portada')) {
      var caja = wrap.querySelector('.buscador-caja');
      caja.addEventListener('click', function (e) {
        if (!wrap.classList.contains('expandido')) {
          wrap.classList.add('expandido');
          input.focus();
        }
      });
      document.addEventListener('click', function (e) {
        if (wrap.classList.contains('expandido') && !wrap.contains(e.target) && !input.value.trim()) {
